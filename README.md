...
**v2.0.0**: HeartMuLa公式論文（arXiv:2601.10547）の仕様に完全準拠。タグ体系を強化し、選択確率による優先度を明示化しました。

## 機能

- **キーワード入力**: 自然言語のキーワードを入力
- **自動タグ生成**: HeartMuLa等に対応したスタイルタグを自動生成（選択確率準拠）
- **歌詞生成**: 構造タグと細粒度タグを含む完全な歌詞を生成
...
構造タグの後に追加して、セクションごとの詳細な表現を制御：

- **ダイナミクス**: Moderate intensity, High energy sustain, Explosive
- **ボーカル**: Introspective vocal delivery, Triumphant vocal expression
- **雰囲気**: Atmospheric build, Narrative progression, Gradual fade

例: `[Chorus] [Explosive melodic hook, high energy sustain, triumphant vocal expression]`
...
**生成される歌詞例:**
```
[Intro] [Soft piano melody, atmospheric build, gentle mood]

[Verse] [Introspective vocal delivery, subtle warmth]
窓の外 舞い落ちる雪
カフェの香り 優しく包む
...
```
